...
Skip this section if you ran `npx svelte-devtools init`. The project also needs its normal Svelte compiler integration. For plain Vite, use `@sveltejs/vite-plugin-svelte@^7` with Svelte 5 and Vite 8. SvelteKit projects use their existing `sveltekit()` integration.
...
Keep the generated `sveltekit(...)` call and all its options. Add `DevTools()` before it and `svelteDevTools()` after it. Import those two functions as shown above. Add the development hook:
